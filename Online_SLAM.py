...
def online_slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    for k in range(len(data)):
        ...
        # update the information maxtrix/vector based on the robot motion, change this part
        ...
        Omega.value[2][0]-=1.0 / motion_noise
        Omega.value[0][2]-=1.0 / motion_noise
        Omega.value[2][2]+=1.0 / motion_noise
        
        Omega.value[1][1]+=1.0 / motion_noise
        Omega.value[3][1]-=1.0 / motion_noise
        Omega.value[1][3]-=1.0 / motion_noise
        Omega.value[3][3]+=1.0 / motion_noise
        
        B=Omega.take([0,1], [0,1])
        A=Omega.take([0,1],range(2, dim+2))
        
        Omega_a=Omega.take(range(2, dim+2), range(2, dim+2))
        
        
        
        Omega=Omega_a-A.transpose()*B.inverse()*A
        
        
        Xi=Xi.expand(dim+2, 1, a, [0])
        Xi.value[0][0]+=-motion[0] / motion_noise
        Xi.value[2][0]+=motion[0] / motion_noise
        
        Xi.value[1][0]+=-motion[1] / motion_noise
        Xi.value[3][0]+=motion[1] / motion_noise
        
        Xi_a=Xi.take(range(2, dim+2), [0])
        
        C=Xi.take([0,1],[0])
        
        
        Xi=Xi_a-A.transpose()*B.inverse()*C
