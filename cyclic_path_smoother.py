...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    while delta_x>tolerance or delta_y>tolerance:
        ...
        for i in range(len(newpath)):
            old_x=newpath[i][0]
            old_y=newpath[i][1]
            newpath[i][0]=newpath[i][0]+weight_data*(path[i][0]-newpath[i][0]) +weight_smooth*(newpath[(i+1)%len(newpath)][0]+newpath[(i-1)%len(newpath)][0]-2*newpath[i][0])

            newpath[i][1]=newpath[i][1]+weight_data*(path[i][1]-newpath[i][1]) +weight_smooth*(newpath[(i+1)%len(newpath)][1]+newpath[(i-1)%len(newpath)][1]-2*newpath[i][1])


            delta_x=delta_x+abs(newpath[i][0]-old_x)
            delta_y=delta_y+abs(newpath[i][1]-old_y)
    #
    # Enter code here
    #
