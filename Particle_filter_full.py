...
class robot:
    ...
    def move(self, motion): # Do not change the name of this function
        ...
        if motion[1]<0.0:
            raise ValueError, 'Moving backwards is invalid'
        #make a new copy
        res=robot()
        ...
        #apply noise
        steering2=random.gauss(motion[0], self.steering_noise)
        distance2=random.gauss(motion[1], self.distance_noise)


        beta=distance2*tan(steering2)/res.length
        if beta>0.1:
            R=motion[1]/beta
            ...
            res.x=self.x+motion[1]*cos(self.orientation)
            res.y=self.y+motion[1]*sin(self.orientation)
            res.orientation=(self.orientation+beta)%(2*pi)
    ...
    # sense:
    #
    def sense(self,optionalFlag=True): #do not change the name of this function
        Z = []
        for i in range(len(landmarks)):
            dely=landmarks[i][0]-self.y
            delx=landmarks[i][1]-self.x
            bearing=atan2(dely, delx)-self.orientation
            if optionalFlag:
